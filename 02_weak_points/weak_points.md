...
## 問題37（P2）｜ドメイン6（セキュリティ）｜難易度：中
**概要：** 指定されたIPアドレス範囲外からのすべてのAWSアクションをブロックしたい、組織全体に適用したい場合、DOPとしてどうするべきか？
**サービス：SCP / Firewall Manager / Network Firewall**
**理由：Firewall Manager + Network Firewall を選んだ。これはVPCトラフィック制御でAPI制御ではない**
**ポイント：**
- AWS APIアクションをブロック → SCP（aws:SourceIp 条件）
- Firewall Manager / Network Firewall = VPC内外のネットワークトラフィック制御。API呼び出しは制御不可
- SCP は Allow ではなく Deny を使う（明示的 Deny は他のポリシーで上書き不可）

---

## 問題33（P2）｜ドメイン2（IaC・設定管理）｜難易度：中
**概要：** Auto Scalingグループで起動したEC2インスタンスに起動後即座にOS設定を適用したい場合、DOPとしてどうするべきか？
...
**概要：** CloudWatch Logs → Kinesis → Lambda → S3 のログ処理パイプラインで発生している遅延を最小限に抑えたい場合、DOPとしてどうするべきか？
...
**概要：** 各チームが自身のパッケージを管理したい、共有ライブラリを組織全体でアクセスできるようにしたい、管理労力を最小化したい場合、DOPとしてCodeArtifactをどう設計するべきか？
...
**概要：** 2リージョンのS3双方向レプリケーションで失敗したオブジェクトを再試行したい、データ量が大規模な場合、DOPとしてどうするべきか？
...
**概要：** CloudFormationで作成するS3バケットにKMS暗号化を強制したい、暗号化なしのバケット作成を未然に防ぎたい場合、DOPとしてどうするべきか？
...
**概要：** 管理アカウントで動くLambdaを専用アカウントに移行した後も、OrganizationsのAPIでAWSアカウントを作成し続けたい場合、DOPとしてどうするべきか？
...
**概要：** 開発者がCloudFormation経由のみでインフラ変更できるようにしたい、コンソールやCLIからの直接操作を禁止したい場合、DOPとしてどうするべきか？
...
**概要：** EKS上のアプリのメモリメトリクスを収集・分析したい、CPUだけでなくメモリを基にスケーリングしたい場合、DOPとしてどうするべきか？
...
**概要：** Network FirewallのフローログをリアルタイムでS3に保存したい、保存前に追加情報を組み込んで加工したい場合、DOPとしてどうするべきか？
...
**概要：** 中央DevOpsアカウントのCodeBuildから、各チームのAWSアカウント内のEKSクラスターにクロスアカウントでデプロイしたい場合、DOPとしてどうするべきか？
...
**概要：** CodeBuildでDockerイメージをビルドしてECRにプッシュしたい、S3から移行したらアクセスが失敗した場合、DOPとして何が不足しているか？
...
**概要：** KMS暗号化されたアーティファクトを使ってクロスアカウントのCodePipelineからCloudFormationデプロイしたい、アクセス拒否エラーを解消したい場合、DOPとしてどうするべきか？
...
**概要：** 同一CIDR（192.168.0.0/22）を持つ20アカウントのVPC間でプライベート通信したい、既存環境の変更を最小限にしたい場合、DOPとしてどうするべきか？
...
**概要：** Config の restricted-ssh ルール違反を検知したい、違反したSGの名前・IDを含む通知をSNSに送りたい場合、DOPとしてどうするべきか？
...
**概要：** CodeDeployがAllowTrafficライフサイクルステージで失敗する、エラーログに理由が記録されていない場合、DOPとして何が原因か？
...
**概要：** EC2・IoT Greengrassデバイス・オンプレミスサーバーを SSM で統合管理したい、パッチ適用を自動化したい場合、DOPとしてどうするべきか？
...
**概要：** Config のルールセットと修復アクションを組織全体に標準化したい、メンバーアカウントが変更できないようにしたい場合、DOPとしてどうするべきか？
...
**概要：** 組織内のすべてのALB・API GatewayにWAF Web ACLを強制適用したい、設定漏れを未然に防ぎたい場合、DOPとしてどうするべきか？
...
**概要：** 別大陸に新オフィスを開設後、遠隔地のユーザーの遅延を削減したい、アプリケーションの可用性を向上させたい場合、DOPとしてどうするべきか？
...
**概要：** Lambda関数のカナリアデプロイをIaCで定義したい、CI/CDパイプラインで自動化したい、問題発生時は自動ロールバックしたい場合、DOPとしてどうするべきか？
